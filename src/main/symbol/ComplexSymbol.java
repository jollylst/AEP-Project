 ...
 */
public class ComplexSymbol implements Symbol{
    protected List<Integer> params;
    private static final double WIDTH = 0.05;

    public ComplexSymbol(List<Integer> params) {
        if (params == null) {
    ...
    public static ComplexSymbol randomComplexSymbol() {
        int N = StdRandom.uniform(1, 10);
        ArrayList<Integer> params = new ArrayList<>(N);
        for (int i = 0; i < N; i += 1) {
            params.add(StdRandom.uniform(0, 255));
        }
        return new ComplexSymbol(params);
    }
    ...
    public boolean equals(Object o) {
        ...
        ComplexSymbol otherComplexSymbol = (ComplexSymbol) o;
        return params.equals(otherComplexSymbol.params);
    }

    public static void main(String[] args) {
        System.out.println("Drawing 4 random complex symbols.");
        randomComplexSymbol().draw(0.25, 0.25, 1.5);
        randomComplexSymbol().draw(0.75, 0.75, 1.5);
        randomComplexSymbol().draw(0.25, 0.75, 1.5);
        randomComplexSymbol().draw(0.75, 0.25, 1.5);
    }
}
